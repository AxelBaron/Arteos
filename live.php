...
<div id="ariane">
	<p>Vous êtes ici : Photos > <a href="live.php">Lives</a></p>
</div>
<?php include('includes/reseauxsociaux.php'); ?>
<?php include('includes/champrecherche.php'); ?>
<script src="js/scrollPlusLive.js"></script>
	...
	<div id="rechercheavancee">
		...
		<form id="formavance" action="resultatlive.php" method="GET">
			<fieldset class="souscat">
				<legend>Rechercher dans les lives</legend>
	...
	<section class='recepteur'>
		...
		$sql="SELECT * FROM evenement_individuel WHERE _id_type_indiv = 8 ORDER BY id_event DESC LIMIT 12";
		$liste = $pdo->query($sql);
        ...
        $sql="SELECT * FROM evenement_individuel WHERE _id_type_indiv = 8 ORDER BY id_event DESC";
		$liste = $pdo->query($sql);
        $data= $liste->fetch();
        if($data == null){
	...
	<div id="btnscrollinfini" onClick="scrollPlusLive()">Afficher plus d'articles</div>
