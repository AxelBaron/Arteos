...
<div id="ariane">
	<p>Vous êtes ici : Photos > <a href="artdescene.php">Art de la scène :</a></p>
</div>
<?php include('includes/reseauxsociaux.php'); ?>
<?php include('includes/champrecherche.php'); ?>
<script src="js/scrollPlusArtdescene.js"></script>
	...
	<div id="rechercheavancee">
		...
		<form id="formavance2" action="resultatartdescene.php" method="GET">
			<fieldset class="souscat">
				<legend>Rechercher dans l'Art de la scène</legend>
        		<input type="search" placeholder="Rechercher une course, un sport..." name="recherche" />
	...
	<section class='recepteur'>
		...
		$sql="SELECT * FROM evenement_individuel WHERE _id_type_gp = 7  ORDER BY id_event DESC LIMIT 12";
		$liste = $pdo->query($sql);
		while ($data= $liste->fetch()) {
        ...
        $sql="SELECT * FROM evenement_individuel WHERE _id_type_gp = 7 ORDER BY id_event DESC";
		$liste = $pdo->query($sql);
        $data= $liste->fetch();
        if($data == null){
	...
	<div id="btnscrollinfini" onClick="scrollPlusArtdescene()">Afficher plus d'articles</div>
