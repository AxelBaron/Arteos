<?php
	...
	$sql="SELECT * FROM artiste WHERE nom LIKE '%$recherche%'";
...
<div id="liste-elements">
	<?php
		...
		$resultat = $pdo->query($sql);
		$nbEntre = 0;
		echo "Résultat(s) pour votre recherche : $recherche <br/> <br/> <a class='btn-retour' href='gestion-artiste.php'>Retour à la liste complète</a><br /><br />";
		while($donnees = $resultat->fetch()){
			$nbEntre++;
			$pasElements = false;
			...
			echo($donnees["nom"]);
			echo("<a class='btn_modif' href='form-modifier-photographe.php?id_artiste=".$donnees['id_artiste']."'>Modifier</a>");
			echo("</div>");
		}
