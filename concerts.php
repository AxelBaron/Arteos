...
<div id="ariane">
	<p>Vous êtes ici : Photos > <a href="concerts.php">Concerts</a></p>
</div>
<?php include('includes/reseauxsociaux.php'); ?>
<?php include('includes/champrecherche.php'); ?>
<script src="js/scrollPlusConcert.js"></script>
	...
	<section class='recepteur'>
		...
		$sql="SELECT * FROM evenement_individuel WHERE _id_type_indiv = 1 ORDER BY id_event DESC LIMIT 12";
		$liste = $pdo->query($sql);
        ...
        $sql="SELECT * FROM evenement_individuel WHERE _id_type_indiv = 1 ORDER BY date DESC";
		$liste = $pdo->query($sql);
        $data= $liste->fetch();
        if($data == null){
	...
	<div id="btnscrollinfini" onClick="scrollPlusConcert()">Afficher plus d'articles</div>
